...
interface Technique { label: string; energy: number; width: number; damage: number; ranged?: boolean }
...
const TECHNIQUES: Record<Exclude<BaseCharacter['definition']['id'], 'yuta'>, Technique[]> = {
  ...
  gojo: [
    { label: '술식 순전', energy: 15, width: 420, damage: 20, ranged: true }, { label: '아오', energy: 20, width: 500, damage: 28, ranged: true }, { label: '아카', energy: 22, width: 460, damage: 32, ranged: true }, { label: '무하한', energy: 18, width: 360, damage: 22, ranged: true }, { label: '허식 자', energy: 34, width: 620, damage: 42, ranged: true },
  ],
  sukuna: [
...
export class CharacterSkillSystem {
  ...
  cast(index: 0 | 1 | 2 | 3 | 4, owner: BaseCharacter, opponent: BaseCharacter, now: number): boolean {
    ...
    if (technique.ranged) this.combat.rangedStrike(owner, opponent, technique.width, technique.damage, now, technique.label)
  ...
  reverseTechnique(owner: BaseCharacter): boolean {
    if (owner.definition.id === 'yuta' || !owner.spendEnergyPercent(0.42)) return false
