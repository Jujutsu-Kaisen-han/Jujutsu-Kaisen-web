...
export function BattleHUD({ state }: BattleHUDProps): ReactElement {
  ...
  return <div className="battle-hud"><div className="battle-top"><FighterPanel fighter={p1} side="p1" /><div className="round-clock"><span>ROUND {state?.round ?? 1}</span><strong>{Math.ceil(state?.timeLeft ?? 90).toString().padStart(2, '0')}</strong><small>{state?.p1Rounds ?? 0} — {state?.p2Rounds ?? 0}</small></div><FighterPanel fighter={p2} side="p2" /></div>{state?.roundMessage && <div className="round-banner">{state.roundMessage}</div>}<div className="battle-bottom"><span><b>LMB</b> BASIC</span><span><b>RMB</b> RCT {cooldownText(p1.cooldowns.reverse)}</span><span><b>1 - 5</b> SKILLS</span><span><b>F</b> GUARD</span><span><b>G</b> SIMPLE DOMAIN {cooldownText(p1.cooldowns.simpleDomain)}</span><span><b>H</b> DOMAIN {cooldownText(p1.cooldowns.domain)}</span><span><b>W</b> JUMP</span><span><b>A / D</b> MOVE</span><span><b>← →</b> P2 MOVE</span><span className="phase-note">CD 1:{cooldownText(p1.cooldowns.skill1)} 2:{cooldownText(p1.cooldowns.skill2)} 3:{cooldownText(p1.cooldowns.skill3)} 4:{cooldownText(p1.cooldowns.skill4)} 5:{cooldownText(p1.cooldowns.skill5)}</span></div></div>
}
