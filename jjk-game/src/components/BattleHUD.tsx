...
export function BattleHUD({ state }: BattleHUDProps): ReactElement {
  const p1 = state?.p1 ?? { name: 'P1', hp: 260, maxHp: 260, energy: 100, maxEnergy: 100, ultimate: 0, ultimateReady: false, domainActive: false, simpleDomainActive: false, fullManifestActive: false, guard: false, combo: 0, cooldowns: emptyCooldowns }
  const p2 = state?.p2 ?? { name: 'P2', hp: 270, maxHp: 270, energy: 100, maxEnergy: 100, ultimate: 0, ultimateReady: false, domainActive: false, simpleDomainActive: false, fullManifestActive: false, guard: false, combo: 0, cooldowns: emptyCooldowns }
  return <div className="battle-hud"><div className="battle-top"><FighterPanel fighter={p1} side="p1" /><div className="round-clock"><span>ROUND {state?.round ?? 1}</span><strong>{Math.ceil(state?.timeLeft ?? 90).toString().padStart(2, '0')}</strong><small>{state?.p1Rounds ?? 0} — {state?.p2Rounds ?? 0}</small></div><FighterPanel fighter={p2} side="p2" /></div>{state?.roundMessage && <div className="round-banner">{state.roundMessage}</div>}<div className="battle-bottom"><span><b>P1 LMB</b> BASIC</span><span><b>P1 RMB</b> RCT {cooldownText(p1.cooldowns.reverse)}</span><span><b>P1 E R T Y U</b> SKILLS</span><span><b>P1 F</b> GUARD</span><span><b>P1 G</b> SIMPLE DOMAIN {cooldownText(p1.cooldowns.simpleDomain)}</span><span><b>P1 H</b> DOMAIN</span><span><b>P2 1</b> BASIC</span><span><b>P2 2</b> GUARD</span><span><b>P2 3</b> SIMPLE DOMAIN</span><span><b>P2 4 - 8</b> SKILLS</span><span><b>P2 9</b> RCT</span><span><b>P2 0</b> DOMAIN</span><span><b>W / ↑</b> JUMP</span><span><b>A / D / ← →</b> MOVE</span><span className="phase-note">CD E:{cooldownText(p1.cooldowns.skill1)} R:{cooldownText(p1.cooldowns.skill2)} T:{cooldownText(p1.cooldowns.skill3)} Y:{cooldownText(p1.cooldowns.skill4)} U:{cooldownText(p1.cooldowns.skill5)}</span></div></div>
}
