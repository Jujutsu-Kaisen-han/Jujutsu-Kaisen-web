...
- 티어별 캐릭터 보드
- 티어표 편집 모드, 개인 티어 배치 로컬 저장, 기본 티어 복원
- 캐릭터 목록, 한글/영문/초성 검색, 특성/분류/역할/즐겨찾기 필터, 공유 가능한 URL 필터
- 캐릭터 상세 페이지, 로컬 저장소 기반 즐겨찾기, 필터 맥락을 유지하는 이전/다음 캐릭터 이동, 스킬/필살기/공식 소개 영상
...
- `/` - 팬텀 퍼레이드 티어표
- `/characters` - 캐릭터 도감과 검색/필터 (`q`, `trait`, `category`, `role`, `sort`, `favorites` 쿼리 지원)
- `/characters/:characterId` - 캐릭터 상세
- `/series` - 작품 정보

티어표에서 `티어 편집`을 누르면 각 캐릭터의 SS/S/A/B/C 티어를 직접 바꿀 수 있습니다. 변경 내용은
브라우저의 로컬 저장소에 저장되며, `기본 티어 복원`으로 초기 데이터로 되돌릴 수 있습니다.
