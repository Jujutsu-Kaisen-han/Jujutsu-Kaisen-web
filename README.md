...
- 티어표와 캐릭터 목록의 compact 카드(작은 이미지 + 이름) 및 카드 클릭 상세 이동
- 티어표에 넣을 캐릭터 목록, 한글/영문/초성 검색, 특성/분류/역할/즐겨찾기 필터
- 캐릭터 상세 페이지, 로컬 저장소 기반 즐겨찾기, 스킬/필살기/공식 소개와 공식 영상
...
저장됩니다. `전체 배치 초기화`로 모든 캐릭터를 다시 미배치 상태로 되돌릴 수 있습니다. 카드 앞면에는
작은 이미지와 이름만 표시하며, 카드를 누르면 상세 페이지에서 설명·스킬·필살기와 연결된 공식 영상을
확인할 수 있습니다.
티어표의 `빠른 배치` 선택 메뉴로도 같은 작업을 할 수 있어 드래그앤드롭을 사용하기 어려운 환경에서도
등급을 조정할 수 있습니다.
